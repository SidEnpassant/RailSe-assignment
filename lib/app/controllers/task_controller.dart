...
class TaskController extends GetxController {
  ...
  void updateTaskDate(
    ...
    try {
      ...
      if (taskIndex != -1) {
        ...
        if (isDeadline) {
          updatedTask = tasks[taskIndex].copyWith(deadline: newDate);
        } else {
          updatedTask = tasks[taskIndex].copyWith(startDate: newDate);
        }
        tasks[taskIndex] = updatedTask;

        Get.snackbar(
          isDeadline ? 'Deadline Updated' : 'Start Date Updated',
          isDeadline
              ? 'Task deadline has been updated successfully'
              : 'Task start date has been updated successfully',
          snackPosition: SnackPosition.TOP,
          backgroundColor: Colors.blue,
          colorText: Colors.white,
          duration: Duration(seconds: 2),
  ...
  Future<void> showDatePicker(
    ...
    final DateTime? pickedDate = await Get.dialog<DateTime>(
      AlertDialog(
        title: Text(isDeadline ? 'Select Deadline' : 'Select Start Date'),
        content: Container(
          height: 300,
          width: 300,
          child: CalendarDatePicker(
            initialDate: currentDate,
            firstDate: isDeadline
                ? DateTime.now()
    ...
    if (pickedDate != null && pickedDate != currentDate) {
      updateTaskDate(taskId, pickedDate, isDeadline: isDeadline);
    }
  }
}
